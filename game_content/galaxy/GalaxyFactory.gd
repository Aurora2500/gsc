...
const solar_system_scene = preload("res://game_content/solar_system/SolarSystem.tscn")
...
var star_sep_squared = star_separation*star_separation
export var tries = 200

export var link_distance = 300
var link_distance_squared = link_distance*link_distance

func _ready():
	rng.randomize()

func get_galaxy():
	return galaxy_scene.instance()

func load_save(savedata):
	var galaxy = get_galaxy()
	for ss_savedata in savedata.solar_systems:
		var current_star: SolarSystem = solar_system_scene.instance()
		galaxy.add_child(current_star)
		galaxy.solar_systems.append(current_star)
		current_star.load_save(ss_savedata)
	return galaxy

func generate_galaxy(empty_galaxy: Galaxy) -> void:
	for i in range(number_of_stars):
		...
		while is_coliding and tries_as_far < tries:
			...
			is_coliding = false
			for other_star in empty_galaxy.solar_systems:
		...
		var current_star: SolarSystem = solar_system_scene.instance()
		empty_galaxy.add_child(current_star)
		empty_galaxy.solar_systems.append(current_star)
		current_star.position = coord
		current_star.id = i
		current_star.ss_name = "Star-" + String(i)
		current_star.link_to(linkable_systems(current_star, empty_galaxy))

func linkable_systems(target: SolarSystem, galaxy: Galaxy) -> Array:
	var linkable_systems_array := []
	for other in galaxy.solar_systems:
		...
		if link_distance_squared < target.position.distance_squared_to(other.position):
			continue
		for intersect_1 in galaxy.solar_systems:
